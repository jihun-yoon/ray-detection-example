...
async def timeit(name, fn, multiplier=1):
    ...
    logger.info("\t{} {} +- {} requests/s".format(name,
                                                  round(np.mean(stats), 2),
    ...
    class Backend:
        ...
        async def batch(self, reqs):
            ...
            for req in reqs:
                ...
                #print(req)
                with open(
                        "/host_server/hdd_ext/hdd_ext_tmp/usr/hutom-admin/data/small-coco/small_coco/train_2017_small/000000005802.jpg",
                        "rb") as image:
                    f = image.read()
                    b = bytearray(f)
                image_payload_bytes = b  #await req.body()
                ...
                # postprocess
                sc_idx = []
                for i, sc in enumerate(output_tensor[0]["scores"]):
                    if sc > 0.5:
                        sc_idx.append(i)
                result = {}
                for k in output_tensor[0]:
                    result[k] = output_tensor[0][k][sc_idx].cpu().numpy(
                    ).tolist()
...
async def main():
    ...
    dump_json(result_json, "result.json")
    return result_json
